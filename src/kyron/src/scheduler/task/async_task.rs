...
mod tests {
    ...
    use crate::{
        ...
        scheduler::{
            ...
            workers::worker_types::WorkerType,
        },
        testing::*,
    };
    ...
    fn test_task_ctor() {
...

        {
            ...
            let scheduler = create_mock_scheduler();
            let worker_id = WorkerId::new("TestWorker".into(), 1, 2, WorkerType::Async);
...

        {
            ...
            let scheduler = create_mock_scheduler();
            let worker_id = WorkerId::new("TestWorker".into(), 0, 2, WorkerType::Async);
            let task = AsyncTask::new(boxed, &worker_id, scheduler);
            let id = task.id();
            assert_eq!(id.worker(), 2); // Test some internals
            assert_eq!(id.engine(), 0);
