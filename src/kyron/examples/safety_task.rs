...
async fn failing_safety_task() -> Result<(), String> {
    info!(
        "Worker-N: failing_safety_task. Engine ID: {:?}, Worker ID: {:?}, Task ID: {:?}",
        TaskContext::engine_id(),
...
async fn passing_safety_task() -> Result<(), String> {
    info!(
        "Worker-N: passing_safety_task. Engine ID: {:?}, Worker ID: {:?}, Task ID: {:?}",
        TaskContext::engine_id(),
        TaskContext::worker_id(),
        TaskContext::task_id().unwrap()
    );
...
async fn passing_non_safety_task() -> Result<(), String> {
    info!(
        "Dedicated worker (dw1): passing_non_safety_task. Engine ID: {:?}, Worker ID: {:?}, Task ID: {:?}",
        TaskContext::engine_id(),
        TaskContext::worker_id(),
        TaskContext::task_id().unwrap()
    );
...
fn main() {
    ...
    runtime.block_on(async move {
        info!(
            "Parent task. Engine ID: {:?}, Worker ID: {:?}, Task Id: {:?}",
            TaskContext::engine_id(),
            TaskContext::worker_id(),
            TaskContext::task_id().unwrap()
        );
